...
**구축한 것:**
- **GCP Cloud Run SSR** — 아파트 상세 페이지에 `getServerSideProps` 구현, AWS Amplify에서 컨테이너 기반 GCP로 마이그레이션 (PR #173)
- **동적 사이트맵: 5개 URL → 48,706개 아파트** — 실시간 API 기반 사이트맵, 우선순위 및 업데이트 빈도 설정 (PR #51)
- **30개 CSR 페이지 → SSR 전환** — 클라이언트 `useMediaQuery`를 서버 사이드 UA 감지로 마이그레이션, 하이드레이션 불일치 해소 (PR #704, -2,347줄)
- **SSR 안티패턴 제거** — 33개 페이지에서 `typeof window === 'undefined'` 가드 정리 (PR #548)
...
**구축한 것:**
- **OG 메타 태그 중앙화** — 단일 유틸리티에서 35개 이상 라우트를 하나의 소스로 관리 (PR #581)
- **SSR OG 타임아웃 수정** — 아파트 상세 API (60개 이상 필드) → 메타 전용 경량 API로 교체 (PR #597)
- **10개 누락 페이지 OG 태그 추가** — AI 검색, 매매/전세 리스팅, 중개사 온보딩, 오피스 딜 포함 (PR #548)
- **SEO 검색 스니펫 개선** — 키워드 정제된 title/description (PR #156)
...
주요 최적화: Naver Maps SDK 지연 로딩 (#517), LCP 이미지 최적화 (#641), 다이나믹 임포트 + react-spring 제거 (#699), 스켈레톤 로딩 (#701), 이미지 명시적 크기 지정으로 CLS 수정 (#621).
