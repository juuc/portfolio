...
부톡의 모든 클라이언트 애플리케이션을 통합하는 중앙 프론트엔드 모노레포. **2025년 11월에 생성** — 기존에 분산되어 있던 레포지토리(bootalk_web, bootalk_app, bootalk_admin)를 단일 코드베이스로 통합하여 크로스 레포 의존성 문제를 해결하고 공유 인프라를 가능하게 한 전략적 이니셔티브.
...
- **apps/nonghyup_web** — 농협 파트너 웹뷰
- **apps/lotte_web** — 롯데카드 파트너 웹뷰
...
**임팩트:** 선제적 에러 감지. 각 PR이 특정 Sentry 이슈 ID를 참조하여 프로덕션 인시던트에 직접 대응.
...
- **Phase 2–7:** React 모범사례 — SSR 워터폴 제거, useEffect 의존성 최적화, non-urgent update에 `startTransition` 적용 ([PR #639](https://github.com/uitiorg/frontend-monorepo/pull/639))
- **Phase 5.1+6.1:** LCP 이미지 최적화 및 GTM 통합 ([PR #641](https://github.com/uitiorg/frontend-monorepo/pull/641))
- **Phase 8:** 성능 최적화 및 인증 리팩토링 ([PR #663](https://github.com/uitiorg/frontend-monorepo/pull/663))
- **홈페이지 릴리스:** 홈페이지 전용 성능 최적화 ([PR #678](https://github.com/uitiorg/frontend-monorepo/pull/678))
- **번들 축소:** 다이나믹 임포트, react-spring 제거 ([PR #699](https://github.com/uitiorg/frontend-monorepo/pull/699))
- **Barrel import 정리:** Storybook 10, `_app` 정리 ([PR #691](https://github.com/uitiorg/frontend-monorepo/pull/691))
- **스켈레톤 로딩:** 스켈레톤 로딩 시스템 및 레이아웃 리팩토링 ([PR #701](https://github.com/uitiorg/frontend-monorepo/pull/701))
- **검색 UX:** 맵 SDK 로딩 대기 없이 검색 UI 즉시 표시 ([PR #692](https://github.com/uitiorg/frontend-monorepo/pull/692))
- **상태관리:** Zustand 마이그레이션 기반 아키텍처 정리 ([PR #718](https://github.com/uitiorg/frontend-monorepo/pull/718))
