...
- Explored apartment ranking system (부킹) concept with the business team
- Initial work on bootalk_web, admin, and data infrastructure
...
- Initial contributions to bootalk_app
...
- **First significant frontend commits** — started contributing to bootalk_app and bootalk_web (Aug 16). Until this point, my work was focused on data pipelines and crawlers as CDO. The CTO's departure forced a rapid transition to fullstack ownership.
- Expo framework migration planning and initial execution on bootalk_app
...
- **SEO infrastructure overhaul** — SSR on GCP Cloud Run, dynamic sitemap (5→48,706 URLs), canonical URLs, clean URL migration (`/detailmap`→`/apt`), robots.txt optimization
...
- **OG meta centralization** — unified 35+ routes into single `getMetaInfoByPath.ts`, fixed SSR timeout with lightweight API
...
| 2025-08 | **First frontend commits** — transitioned from CDO (data) to fullstack (bootalk_app, bootalk_web) |
