...
**Key PRs:**
- `feat(agent): add /me/ endpoints for IDOR protection` (#265)
- `feat: monolithic API decomposition - 5 independent endpoints` (#166)

---

## laad-dashboard — Apartment Advertising (LAAD)
**11 commits**
...
## semugpt-frontend-forked — Tax Consulting GPT (세무GPT)
**10 commits**
...
- **Legal Data Pipeline:** Indexed tax laws (조문 단위), precedents (판례), and execution standards (집행기준) into Elasticsearch. Built law-precedent mapping (`tax-threeway` ↔ `tax-laws`), and collected additional sources including 기본통칙 and PDF-based 집행기준
...
## register-poc — Jeonse Fraud Detection (전세사기 탐지)
**3 commits**
...
## error-handler + Openclaw — AI-Powered Error Resolution
**33 commits (error-handler) + new initiative (Openclaw)**

**error-handler:** Automated error analysis system using AI to parse stack traces from Loki logs and notify relevant teams.
...
## manifest-ops, .github, legacy-service, and others
**~13 commits combined**
