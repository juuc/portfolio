...
**What I built:**
- **SSR on GCP Cloud Run** — implemented `getServerSideProps` for apartment detail pages, migrated from AWS Amplify to containerized GCP deployment (PR #173)
- **Dynamic sitemap: 5 URLs → 48,706 apartments** — real-time API-driven sitemap with priority and update frequency (PR #51)
- **30 CSR pages → SSR** — migrated client-side `useMediaQuery` to server-side UA detection, eliminating hydration mismatches (PR #704, -2,347 lines)
- **SSR anti-pattern removal** — cleaned `typeof window === 'undefined'` guards across 33 pages (PR #548)
...
**What I built:**
- **OG meta centralization** — single utility managing 35+ routes from one source of truth (PR #581)
- **SSR OG timeout fix** — replaced heavy apartment detail API (60+ fields) with lightweight API for meta-only data (PR #597)
- **OG tags for 10 missing pages** — including AI search, buy/sell listings, agent onboarding, and office deals (PR #548)
- **SEO snippet improvement** — keyword-refined title/description (PR #156)
...
Key optimizations: Naver Maps SDK lazy loading (#517), LCP image optimization (#641), dynamic imports + react-spring removal (#699), skeleton loading (#701), CLS fix with explicit image dimensions (#621).

> **Epic Issue:** #635 | **Tracking:** #515
