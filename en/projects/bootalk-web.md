...
**Problem:** Duplicate URLs, inconsistent paths (`/detailmap` vs `/apt`), missing canonical tags diluting page authority.

**What I built:**
- **Canonical URL tags** to resolve duplicate URL issues
- **Clean URL migration:** all `/detailmap` paths → `/apt`
...
> Key commits: `467d052`, `cd450d9`, `d4f7304`, `4b9dbe7`, `80efda1`, `48b8754`, `b954ca4`
...
**What I built:**
- **OG meta centralization** — `getMetaInfoByPath.ts` managing 35+ routes from a single source ([monorepo #581](https://github.com/uitiorg/frontend-monorepo/pull/581))
...
- **OG tags for 10 missing pages** — `/aiHouse`, `/apt/buy`, `/apt/sell`, `/agent/start`, `/office/hotdeal` ([monorepo #548](https://github.com/uitiorg/frontend-monorepo/pull/548))
...
| Oct 2025 | Canonical URLs, clean paths, `/detailmap` → `/apt` | multiple commits |
