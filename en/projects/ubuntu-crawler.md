...
**Key PRs:**
- `feat: 실거래가 테이블에 apt_id 부착, 실거래가-아파트정보 간 매핑 프레임워크 제작` (#8)
- `feat: 실거래가 변동사항 확인 및 기록 로직` (#7)
- `feat: Add apt_price_daily_stats table for daily price statistics`
...
**Solution:** Migrated all crawlers to Dagster native assets: AptRanking, Loan, RealPrice. Replaced subprocess calls with native Python. Added async support for performance.
...
**Key PRs:**
- `feat: getrealprice_from_dagster.py를 Dagster 네이티브 asset으로 마이그레이션` (#31)
- `feat(loan): Loan 로직을 Dagster로 완전 마이그레이션` (#30)
- `feat(apt_ranking): AptRanking 로직을 Dagster로 완전 마이그레이션` (#29)
...
**Key PRs:**
- `feat: ./AptRanking에 '아파트 랭킹' 위한 데이터 ETL 로직 구현` (#4)
...
**Key PRs:**
- `feat(bunyang): Enable weekly schedule at Monday 19:00 KST`
- `feat: ./Bunyang 분양 아파트 처리 로직` (#2)
