...
**Decision:** Consolidate `bootalk_web`, `bootalk_app`, `bootalk_admin` + partner apps (`nonghyup_web`, `lotte_web`) into a single Turborepo + pnpm monorepo.
...
frontend-monorepo/
├── apps/
│   ├── bootalk_web/       (Next.js 16)
│   ├── bootalk_app/       (Expo SDK 54)
│   ├── bootalk_admin/     (Next.js 16)
│   ├── nonghyup_web/      ← migrated in
│   └── lotte_web/         ← migrated in
└── packages/
    └── common/            ← unified @bootalk/common
```
...
> References: [Discussion #27](https://github.com/uitiorg/team-discussions/discussions/27) (monorepo milestone proposal), PRs [#737](https://github.com/uitiorg/frontend-monorepo/pull/737) (lotte_web), [#753](https://github.com/uitiorg/frontend-monorepo/pull/753) (nonghyup_web)
...
**Decision:** Consolidate 8 separate Spring Boot microservices (`btalk2.0_*-service`) into a unified modular monolith (`btalk2.1_backend`).
...
```
btalk2.1_backend/
├── agent-module/          ← from btalk2.0_agent-service
├── apart-module/          ← from btalk2.0_data-service
├── ai-module/
├── notification-module/
├── event-module/
├── office-store-module/
...
**Impact:** Full pipeline observability via Dagster UI. Automatic retry with exponential backoff. Run guards prevent hanging jobs (apt_info: 2h max, listing: 1h max). Telegram alerts on failure. Data freshness is now guaranteed, not hoped for.
